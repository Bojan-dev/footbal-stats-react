...
import { hasMatchStarted } from '../../../functions/getFixtureStatus';
...
const SingleFixtureSummary = () => {
  ...
  return (
    <div className={`fixturePadding flexColumnCenter`}>
      {hasMatchStarted(selectedFixture.fixture.status.short) ? (
        <>
          <div
          ...
          {selectedFixture.fixture.status.elapsed > 45 && (
            <>
              ...
              {selectedFixture.events
                ...
                .map((event) => {
                  const eventByTeam =
                    event.team.id === selectedFixture.teams.home.id
                      ? true
                      : false;
                  return (
                    <FixtureSummaryEvent
        ...
        <div>
          <h4> More information will be provided once the match starts!</h4>
        </div>
      )}
    </div>
  );
